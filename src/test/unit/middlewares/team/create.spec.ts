...
import { create } from "../../../../middlewares/team/create";
...
describe("team create middleware", () => {
  it("calls next() with no argument for a valid payload", () => {
    const { next, received } = captureNext();

    create(buildReq({ code: "TEAM-001", name: "Engineering" }), res, next);
  ...
  it("passes ValidationError when 'code' is missing", () => {
    ...
    create(buildReq({ name: "Engineering" }), res, next);
  ...
  it("passes ValidationError when 'code' is not a string", () => {
    ...
    create(buildReq({ code: 123, name: "Engineering" }), res, next);
  ...
  it("passes ValidationError when 'code' is an empty string", () => {
    ...
    create(buildReq({ code: "   ", name: "Engineering" }), res, next);
  ...
  it("passes ValidationError when 'name' is missing", () => {
    ...
    create(buildReq({ code: "TEAM-001" }), res, next);
